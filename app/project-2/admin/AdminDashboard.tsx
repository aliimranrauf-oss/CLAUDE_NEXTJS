...
export default function AdminDashboard() {
  ...
  function handleExport() {
    ...
    a.href = url
    a.download = 'project-2-settings.json'
    a.click()
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-14 sm:px-8">
      {/* Top bar */}
      <div className="p2-card sticky top-[76px] z-30 mb-8 flex flex-wrap items-center justify-between gap-3 p-4 sm:p-5">
        <div>
          <span className="p2-eyebrow">Admin</span>
          <h1 className="p2-display mt-1 text-xl font-semibold text-[var(--p2-navy)]">
            Content &amp; visibility editor
          </h1>
        </div>
        <div className="flex flex-wrap items-center gap-2.5">
          <span className="text-[12px] text-[var(--p2-muted-2)]">
            {saveState === 'saving' ? 'Saving…' : saveState === 'saved' ? 'Saved ✓' : ''}
          </span>
          <button onClick={handleExport} className="p2-btn p2-btn-outline !px-4 !py-2 text-[11px]">
            Export JSON
          </button>
          <button onClick={handleImportClick} className="p2-btn p2-btn-outline !px-4 !py-2 text-[11px]">
            Import JSON
          </button>
          <input ref={fileInputRef} type="file" accept="application/json" onChange={handleImportFile} className="hidden" />
          <button onClick={handleReset} className="p2-btn p2-btn-outline !px-4 !py-2 text-[11px] text-red-500">
            Reset
          </button>
          <Link href="/project-2" target="_blank" className="p2-btn p2-btn-solid !px-4 !py-2 text-[11px]">
            View live site ↗
          </Link>
        </div>
      </div>

      <p className="mb-8 max-w-xl text-[13.5px] leading-relaxed text-[var(--p2-muted)]">
        Changes save automatically to this browser and reflect on the site immediately —
        open <Link href="/project-2" target="_blank" className="p2-underline text-[var(--p2-navy)]">the live page</Link> in
      ...
      <div className="flex flex-col gap-6">
        <AdminSection title="Navigation" description="Show or hide top-nav links, and edit the connect button label.">
          ...
          <Switch label="Expertise" checked={draft.nav.showExpertise} onChange={(v) => update('nav', { showExpertise: v })} />
          <Switch label="Experience" checked={draft.nav.showExperience} onChange={(v) => update('nav', { showExperience: v })} />
          <Switch label="Insights" checked={draft.nav.showInsights} onChange={(v) => update('nav', { showInsights: v })} />
          <Field
            label="Connect button label"
            value={draft.nav.connectLabel}
            onChange={(v) => update('nav', { connectLabel: v })}
            hint="Shown in the header button, e.g. “Let's Connect”."
          />
        </AdminSection>
        ...
        <AdminSection title="Hero — copy" description="The headline, supporting text, and CTA button labels.">
          ...
          <Field
            label="Gold accent word"
            value={draft.hero.accentWord}
            onChange={(v) => update('hero', { accentWord: v })}
            hint="Must exactly match a word within the three heading lines above to be highlighted in gold."
          />
        ...
        <AdminSection title="Hero — cards &amp; credentials" description="Show or hide individual elements in the hero.">
          <Switch label="Credentials row" hint="CPA/CMA, 15+ Years, KSA Based, Vision 2030" checked={draft.hero.showCredentials} onChange={(v) => update('hero', { showCredentials: v })} />
          <Switch label="Revenue Growth card" checked={draft.hero.showRevenueCard} onChange={(v) => update('hero', { showRevenueCard: v })} />
          <Switch label="EBITDA Performance card" checked={draft.hero.showEbitdaCard} onChange={(v) => update('hero', { showEbitdaCard: v })} />
          <Switch label="Budget Performance card" checked={draft.hero.showBudgetCard} onChange={(v) => update('hero', { showBudgetCard: v })} />
          <Switch label="Strategic Focus Areas card" checked={draft.hero.showFocusCard} onChange={(v) => update('hero', { showFocusCard: v })} />
        </AdminSection>
