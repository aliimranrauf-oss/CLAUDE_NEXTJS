...
import { stats, services, experience, testimonials } from './_data/content'
...
function renderHeadingLine(line: string, accentWord: string) {
  ...
  return (
    <>
      {before}
      <span className="text-[var(--p2-gold)]">{accentWord}</span>
      {after}
    </>
  )
}
    ...
    <>
      ...
      <section className="p2-hero relative border-b border-[var(--p2-border)]">
        ...
        <div className="relative z-10 mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-8 sm:py-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:py-24">
          <div>
            <span className="p2-eyebrow p2-fade-up">{h.eyebrow}</span>
            ...
            <div className="p2-fade-up mt-8 flex flex-wrap gap-4" style={{ animationDelay: '0.24s' }}>
              <Link href="/project-2/experience" className="p2-btn p2-btn-solid">
                {h.ctaPrimaryLabel} →
              </Link>
              <Link href="/project-2/contact" className="p2-btn p2-btn-outline">
                {h.ctaSecondaryLabel} →
              </Link>
            </div>
