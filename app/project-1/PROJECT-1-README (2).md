...
- **`HeroPortrait.tsx`** (in `_components/`) — a real photo of Ahmed, framed with brass
  corner brackets, a one-time entrance wipe + brass scan-line sweep on load, and a subtle
  zoom on hover (pure CSS `transform`/`opacity`/`clip-path`, no JS — negligible Lighthouse
  cost). Caption strip underneath now shows only the name (the earlier "PORTRAIT · SCALE
  1:1" sublabel was removed per feedback). Used on the homepage hero and again (cropped
  wider) on the About page. See §7.5 below for image specs.
- A small brass-underlined name plate ("— AHMED AL MANSOORI") now sits above the eyebrow
  line in the hero, filling what was empty space in earlier drafts.
- **`TypewriterHeading.tsx`** — types out the hero headline character-by-character on
  load with a blinking brass cursor. Full text is exposed via `aria-label` immediately
  (screen readers/SEO aren't affected by the animation), and it's skipped entirely for
  `prefers-reduced-motion` users, who see the final text right away.
- **`CountUp.tsx`** — animates the stats-strip numbers (`14+`, `AED 4.6B+`, etc.) counting
  up from 0 once they scroll into view, using `requestAnimationFrame`. Parses the numeric
  portion out of the string automatically and keeps any prefix/suffix (currency codes, `+`,
  units) static. Also respects `prefers-reduced-motion`.
...
│   │                             #   homepage hero + About page. See §7.5 for image specs.
│   ├── TypewriterHeading.tsx    # Client component: types out the hero headline on load
│   └── CountUp.tsx              # Client component: animates stat numbers counting up
│                                 #   when scrolled into view (uses framer-motion's useInView)
│
├── _data/
...
**To adjust the typing/count-up animation speed:**
- `TypewriterHeading` takes optional `speed` (ms per character, default `34`) and
  `startDelay` (ms before typing begins, default `250`) props — passed where it's used
  in `page.tsx`.
- `CountUp` takes an optional `duration` prop (ms for the count to finish, default `1300`)
  — passed where it's used in `page.tsx`.
